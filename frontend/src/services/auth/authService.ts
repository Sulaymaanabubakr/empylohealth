...
export const authService = {
    _pendingMfaResolver: null as MultiFactorResolver | null,
    /**
     * Initialize Google Sign In
     * @param {string} webClientId - From Firebase Console > Auth > Google > Web SDK config
     */
    init: (webClientId: string): void => {
     ...
     * ... (existing)
     */
    login: async (email: string, password: string): Promise<MfaLoginResponse> => {
        try {
     ...
     */
    sendPasswordReset: async (email: string): Promise<SuccessResponse> => {
        try {
     ...
     */
    confirmPasswordReset: async (oobCode: string, newPassword: string): Promise<SuccessResponse> => {
        try {
    ...
    startSmsMfaSignIn: async (hint: MultiFactorInfo, recaptchaVerifier: any): Promise<string> => {
    ...
    resolveSmsMfaSignIn: async (verificationId: string, code: string): Promise<UserCredential> => {
        if (!authService._pendingMfaResolver) {
            throw new Error('No pending MFA resolver.');
        }
    ...
    resolveTotpMfaSignIn: async (hint: MultiFactorInfo, code: string): Promise<UserCredential> => {
        if (!authService._pendingMfaResolver) {
            throw new Error('No pending MFA resolver.');
        }
