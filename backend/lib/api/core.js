...
const escapeXml = (text) => String(text || '')
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&apos;');
const createRecommendationSvgDataUri = (input) => {
...
const deleteCollectionDocs = async (ref, batchSize = 300) => {
    while (true) {
...
const removeUserFromCircle = async (circleId, uid) => {
 ...
 */
exports.deleteCircle = regionalFunctions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');
    const { circleId } = data || {};
    const uid = context.auth.uid;
 ...
 */
exports.deleteChat = regionalFunctions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');
    ...
    try {
        ...
        if (!circleId) {
            ...
            const nextParticipants = Array.isArray(afterSnap.data()?.participants) ? afterSnap.data()?.participants : [];
...
exports.manageMember = regionalFunctions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (roleChangeNotification) {
            ...
            const fcmTokens = Array.isArray(targetUserData?.fcmTokens) ? targetUserData.fcmTokens : [];
            const expoTokens = Array.isArray(targetUserData?.expoPushTokens) ? targetUserData.expoPushTokens : [];
...
exports.getAffirmations = regionalFunctions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const buildDailySet = async () => {
            ...
            const filtered = all.filter((a) => a.isDeleted !== true && (!a.status || ['active', 'published'].includes(a.status)));
...
const ensureDailyAffirmations = async () => {
    ...
    if (affirmationIds.length === 0) {
        ...
        const filtered = all.filter((item) => item.isDeleted !== true && (!item.status || ['active', 'published'].includes(item.status)));
        const picks = pickRandomItems(filtered, 3);
...
const getUserTimezone = (userData) => {
    ...
    if (isValidIanaTimezone(fromTopLevel))
        return fromTopLevel;
    if (isValidIanaTimezone(fromSettings))
        return fromSettings;
    return 'UTC';
};
...
const sendDailyAffirmationsNotification = async (slotIndex, title) => {
    ...
    for (const user of users) {
        ...
        if (user?.settings?.dailyReminders === false)
            continue;
    ...
    for (let i = 0; i < targetUsers.length; i += batchSize) {
        const batch = db.batch();
        targetUsers.slice(i, i + batchSize).forEach((user) => {
            const notifRef = db.collection('notifications').doc();
            batch.set(notifRef, {
                uid: user.id,
                title: slotTitle,
...
exports.sendAffirmationsMorning = (0, scheduler_1.onSchedule)({ schedule: '*/10 * * * *', timeZone: AFFIRMATION_SCHEDULER_TIMEZONE, region: 'europe-west1' }, async () => {
    await sendDailyAffirmationsNotification(0, 'Morning Affirmation');
});
exports.sendAffirmationsAfternoon = (0, scheduler_1.onSchedule)({ schedule: '*/10 * * * *', timeZone: AFFIRMATION_SCHEDULER_TIMEZONE, region: 'europe-west1' }, async () => {
