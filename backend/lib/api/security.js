...
exports.sendCircleDeletedEmails = exports.sendAccountDeletedEmail = exports.recordLoginDevice = exports.changeEmailWithOtp = exports.completeEmailVerificationWithOtp = exports.changePasswordWithOtp = exports.resetPasswordWithOtp = exports.registerWithOtp = exports.verifyOtp = exports.requestOtp = void 0;
const functions = __importStar(require("firebase-functions/v1"));
const admin = __importStar(require("firebase-admin"));
const crypto_1 = require("crypto");
const brevoEmail_1 = require("../services/brevoEmail");
...
const normalizePurpose = (purpose) => {
    ...
    if (Object.values(OTP_PURPOSES).includes(value)) {
        return value;
    }
    throw new functions.https.HttpsError('invalid-argument', 'Invalid OTP purpose.');
};
const hashString = (value) => (0, crypto_1.createHash)('sha256').update(value).digest('hex');
...
const getRequestIdentity = (context, metadata = {}) => {
    const ipRaw = String(context.rawRequest?.headers['x-forwarded-for'] || context.rawRequest?.ip || '').split(',')[0]?.trim() || 'unknown';
...
const purposeNeedsAuth = (purpose) => (purpose === OTP_PURPOSES.CHANGE_PASSWORD ||
    purpose === OTP_PURPOSES.CHANGE_EMAIL ||
    purpose === OTP_PURPOSES.EMAIL_VERIFY);
const buildOtpDocId = (email, purpose) => `${purpose}:${hashString(email)}`;
const logOtpAudit = async (payload) => {
...
const sendOtpEmail = async (email, purpose, code) => {
    const template = brevoEmail_1.mailTemplates.otp({
        purposeLabel: purposeLabel(purpose),
        code,
        expiresInMinutes: OTP_TTL_MINUTES
    });
    await (0, brevoEmail_1.sendTemplateEmail)({
        to: [{ email }],
        template,
        tags: ['otp', String(purpose).toLowerCase()]
    });
};
exports.requestOtp = regionalFunctions.https.onCall(async (data, context) => {
    ...
    if (purpose === OTP_PURPOSES.RESET_PASSWORD && shouldSend) {
        await (0, brevoEmail_1.sendTemplateEmail)({
            to: [{ email }],
            template: brevoEmail_1.mailTemplates.passwordResetRequested(),
...
exports.verifyOtp = regionalFunctions.https.onCall(async (data, context) => {
    const purpose = normalizePurpose(String(data?.purpose || ''));
    let email = normalizeEmail(String(data?.email || ''));
...
exports.registerWithOtp = regionalFunctions.https.onCall(async (data, context) => {
...
exports.resetPasswordWithOtp = regionalFunctions.https.onCall(async (data, context) => {
    if (context.auth?.uid) {
    ...
    await (0, brevoEmail_1.sendTemplateEmail)({
        to: [{ email }],
        template: brevoEmail_1.mailTemplates.passwordChanged(),
        tags: ['security', 'password-changed']
    });
    return { success: true };
});
exports.changePasswordWithOtp = regionalFunctions.https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
...
exports.completeEmailVerificationWithOtp = regionalFunctions.https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');
    }
    const uid = context.auth.uid;
...
exports.changeEmailWithOtp = regionalFunctions.https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');
    }
    const uid = context.auth.uid;
...
exports.recordLoginDevice = regionalFunctions.https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be logged in.');
    }
    const uid = context.auth.uid;
    ...
    const providedDeviceId = String(metadata.deviceId || '').trim() || `${identity.deviceHash}:${identity.ipHash}`;
    ...
    await deviceRef.set({
        ...
        pushTokenHash: hashString(String(metadata.pushToken || '')),
    ...
    if (isNewDevice && email) {
        await (0, brevoEmail_1.sendTemplateEmail)({
            to: [{ email }],
            template: brevoEmail_1.mailTemplates.newLogin({
                deviceLabel,
...
const sendAccountDeletedEmail = async (params) => {
    const email = normalizeEmail(params.email);
    if (!email)
        return;
    await (0, brevoEmail_1.sendTemplateEmail)({
        to: [{ email }],
        template: brevoEmail_1.mailTemplates.accountDeleted(),
        tags: ['account', 'deleted']
    });
};
exports.sendAccountDeletedEmail = sendAccountDeletedEmail;
const sendCircleDeletedEmails = async (params) => {
    const uniqueEmails = Array.from(new Set((params.memberEmails || []).map((e) => normalizeEmail(e)).filter(Boolean)));
    if (!uniqueEmails.length)
        return;
    const template = brevoEmail_1.mailTemplates.circleDeleted({ circleName: params.circleName || 'Circle' });
    const chunkSize = 60;
...
exports.sendCircleDeletedEmails = sendCircleDeletedEmails;
